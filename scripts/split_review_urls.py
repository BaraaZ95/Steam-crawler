...
def parse_args():
    ...
    parser.add_argument(
        "--output-dir", help="Directory in which to place url part files."
    )
    parser.add_argument("--pieces", help="Number of URL files to produce.", default=10)
...
def main():
    ...
    with open(args.scraped_products) as f:
        rows = [json.loads(l) for l in f]  # noqa: E741
    ...
    for n_part, bound in enumerate(range(0, n, step), start=1):
        file_name = os.path.join(
            args.output_dir, "review_urls_{:02d}.txt".format(n_part)
        )
